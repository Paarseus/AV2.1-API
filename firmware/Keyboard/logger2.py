...
def on_press(key):
    ...
    try:
        ...
        if key == keyboard.Key.space:
            ...
            state['throttle'] = 0.0 if state['throttle'] == 0.5 else 0.5
            send_command(f"T {state['throttle']:.0f}")
            print(f"Throttle: {'ON' if state['throttle'] == 0.5 else 'OFF'}")
            ...
            if c == 'c':
                state['steer'] = 0.0
                send_command(f"S 0")
                print(f"Center: {'ON' if state['steer'] == 0 else 'OFF'}")
