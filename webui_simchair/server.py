#!/usr/bin/env python3
"""
AV Console - Web UI Server with WebRTC Camera Streaming + Sim Chair Support
...
UDP listener for sim chair control with automatic priority switching.
"""
...
# Sim chair UDP configuration
SIMCHAIR_PORT = 5006
SIMCHAIR_TIMEOUT = 0.5  # seconds

# Sim chair state (thread-safe)
simchair_lock = threading.Lock()
simchair_state = {
    "active": False,
    "last_time": 0,
    "s": 0.0,  # steering
...
# Sim chair UDP listener
def simchair_listener():
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.bind(('', SIMCHAIR_PORT))
    sock.settimeout(0.1)
    print(f"[SIMCHAIR] Listening on UDP port {SIMCHAIR_PORT}")
    ...
    while True:
        try:
            ...
            if len(parts) == 4:
                with simchair_lock:
                    simchair_state["s"] = float(parts[0])
                    simchair_state["t"] = float(parts[1])
                    simchair_state["b"] = float(parts[2])
                    # Handle mode as int (truncate if float like "1.0")
                    simchair_state["m"] = {0: 'N', 1: 'D', 2: 'S', 3: 'R'}.get(
                        int(float(parts[3])), 'N')
                    simchair_state["last_time"] = time.time()
                    simchair_state["active"] = True
        except socket.timeout:
            pass
        except Exception as e:
            print(f"[SIMCHAIR] Error: {e}")

        # Check timeout
        with simchair_lock:
            if time.time() - simchair_state["last_time"] > SIMCHAIR_TIMEOUT:
                simchair_state["active"] = False


threading.Thread(target=simchair_listener, daemon=True).start()
    ...
    try:
        while True:
            ...
            if msg_type == "control":
                ...
                # Check sim chair priority
                with simchair_lock:
                    use_simchair = simchair_state["active"]
                    if use_simchair:
                        v = {
                            "t": min(simchair_state["t"], MAX_THROTTLE),
                            "b": simchair_state["b"],
                            "s": simchair_state["s"]
                        }
                        ctrl_mode = simchair_state["m"]
                ...
                actuator.send_all(estop=estop, throttle=v["t"], mode=ctrl_mode,
                                  brake=v["b"], steer=v["s"])
                state.update(v)
                state["m"] = ctrl_mode  # Show actual mode being used
                state["source"] = "simchair" if use_simchair else "phone"
